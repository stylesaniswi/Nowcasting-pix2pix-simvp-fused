...
PREPROCESS_SCALE_SEVIR = {'vis': 1,  # Not utilized in original paper
                          'ir069': 1 / 1174.68,
                          'ir107': 1 / 2562.43,
                          'vil': 1, # changing it from 1 / 47.54 to 1 since dont need to normalize groundtruth
                          'lght': 1 / 0.60517}
PREPROCESS_OFFSET_SEVIR = {'vis': 0,  # Not utilized in original paper
                           'ir069': 3683.58,
                           'ir107': 1552.80,
                           'vil': 0, # changin from - 33.44 to 0 not normalizing y values
                           'lght': - 0.02990}
...
# SEVIR_CATALOG = "Project_model/sample_catalog.csv" # use this catalog for sample 10% of uniform data of radar images only
...
class SEVIRDataLoader:
    ...
    Event Frames:
        ...
        <--stride-->[-----seq_len-----]
                    <--stride-->[-----seq_len-----]
                                        ...
    """
    def get_monthly_sample(self, data, type_column='img_type', timestamp_column='time_utc', sample_fraction=0.1):
        """
        Takes the data and samples 10% of data from each month uniformly.

        Parameters:
        - data (pd.DataFrame): DataFrame containing the data with a timestamp column.
        - timestamp_column (str): The column name containing the timestamp.
        - sample_fraction (float): Fraction of data to sample from each month.

        Returns:
        - pd.DataFrame: DataFrame containing the sampled data.
        """
        # Filter the data for the required type
        data = data[data[type_column] == 'vil']
    
    # Convert the timestamp column to datetime
    
        data[timestamp_column] = pd.to_datetime(data[timestamp_column])
        data['month'] = data[timestamp_column].dt.to_period('M')

        monthly_samples = []
        grouped = data.groupby('month')

        for month, group in grouped:
            sample = group.sample(frac=sample_fraction, random_state=1)
            monthly_samples.append(sample)

        sampled_data = pd.concat(monthly_samples).drop(columns=['month'])
        return sampled_data
    ...
    def __init__(self,
        ...
        if isinstance(sevir_catalog, str):
            ...
            #for sample data 10% uniformly collected------ no issue for only one channel
            # self.catalog = self.get_monthly_sample(self.catalog, sample_fraction=0.1)
            # self.catalog.to_csv('sample_catalog.csv', index=False)
            # print(self.catalog)
        else:
    ...
    def _compute_samples(self):
        ...
        imgts = set(imgt)
        filtcat = self.catalog[ np.logical_or.reduce([self.catalog.img_type==i for i in imgt]) ]  # (39210,21) 过滤保留vil
    ...
    @staticmethod
    def preprocess_data_dict(data_dict, data_types=None, layout='NHWT', rescale=None):
        """
        Parameters
        ----------
    ...
    @staticmethod
    def process_data_dict_back(data_dict, data_types=None, rescale=None):
        """
        Parameters
        ----------
        data_dict
